...
if(isset($_POST["id"])){
    ...
    $sql0 = "SELECT count(*) t FROM clientes WHERE id NOT IN ($id) AND (email='$e' OR telefono='$t');";
    $datos0=$conexionBD->conexion->query($sql0);
    $fila = mysqli_fetch_assoc($datos0);
    $cant=$fila['t'];

    if($cant==0){
        $sql = "UPDATE clientes SET nombres='$n',apellidos='$a',email='$e',telefono='$t', direccion='$d' WHERE id=$id;";
